...
type CheckinRecord struct {
	...
	// IsMakeup marks a day covered retroactively. Such a row always carries
	// QuotaAwarded = 0 — it keeps the streak alive and pays nothing — so a
	// caller that renders the amount alone would print a misleading "+0".
...
type CheckinStats struct {
	...
	// TotalMakeups is how many of TotalCheckins were retroactive make-ups
	// (which cover a day but grant nothing).
...
// CheckinMakeup describes the retroactive make-up affordances the server
// computes. EligibleDates is authoritative: the window can reach into the
// previous month, which the month-scoped Records list cannot express, so a
// caller must never derive eligibility from Records.
...
// CheckinMakeupResult is what DoCheckinMakeup returns. There is no quota field
// by design — a make-up never pays out.
...
// DoCheckinMakeup covers a missed day so the streak survives. date is a UTC
// "YYYY-MM-DD" that must appear in CheckinStatus.Makeup.EligibleDates — the
// server enforces the window, the monthly cap, and the account's registration
// date, and rejects anything else. Grants no quota.
