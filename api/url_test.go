...
// TestWebOriginFromBase locks in the api→app subdomain rewrite. Two behaviours that have bitten us in the past, both covered explicitly:
//
//   - the http:// branch must rewrite too — before the consolidation in #246, cmd/status.go silently passed http://api.* through unchanged while the MCP server rewrote it, so a self-host setup printed the wrong /wallet URL only in `everyapi status`;
//   - only "api." as a leading subdomain matches — bases that happen to start with "api" (like https://api2.example.com or https://apiserver.example.com) MUST pass through, otherwise the rewrite produces nonsense hosts.
