...
// OpenAIProtocol covers the OpenAI Chat Completions wire format and its compatible spinoffs (Codex CLI, almost every "OpenAI-compatible" SDK on the planet, including Anthropic-on-OpenAI shims and DeepSeek).
//
// Sanitisable text fields:
//
//   - messages[].content                — flat string body
//   - messages[].content[].text         — content-blocks variant
//   - messages[].tool_calls[].function.arguments — stringified JSON arguments the model emitted; can carry user-pasted secrets on follow-up turns
...
//
//   - model, temperature, max_tokens, stream, top_p, frequency_penalty, presence_penalty, stop, seed, user (an arbitrary id), tool_choice, etc.
//
// The walkJSON helper matches by key name; we list every text-bearing key here and rely on the walk to find each leaf wherever it lives.
...
func (p *OpenAIProtocol) PathMatch(path string) bool {
	// Match the chat-completions surface and adjacent legacy surfaces that share the same body shape. Embeddings has `input` which can be a string OR array — we treat input as text too.
...
// openaiTextKeys is the registry of JSON object keys whose string value carries user-supplied text. Add to this set if a new field surfaces in OpenAI's spec — schema-driven rather than path-driven to stay robust against minor API revisions.
